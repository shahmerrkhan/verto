...
export default async function handler(req, res) {
  ...
  if (action === 'detail' && id) {
    try {
      const rows = await sql`SELECT * FROM articles WHERE id = ${id} AND status = 'published'`
  ...
  if (action === 'submit' && req.method === 'POST') {
    ...
    if (!body) return
    const { authorName, title, excerpt, content } = body
  ...
  try {
    const rows = await sql`SELECT * FROM articles WHERE status = 'published' ORDER BY published_at DESC`
